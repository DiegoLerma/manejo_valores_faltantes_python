# valores_faltantes guide installation
...
activate manejo_valores_faltantes
```
...
To move beyond notebook prototyping, all reusable code should go into the `valores_faltantes_module/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `valores_faltantes_module` folder and use the modules inside your notebooks :
...
```python
from valores_faltantes_module.utils.paths import data_dir
data_dir()
```
